...
def main():
    ...
    if args.sgs:
        import dependence
        logger.info("Using Spatial Gradient Scaling")
        spatial_gradient_scalings = {}
        with torch.no_grad():
            # set default spatial gradient scaling of all ones
            i = 0
            for param in model.parameters():
                if param.dim() == 4 and param.shape[2] > 1 and param.shape[3] > 1: # if convolution (4d weights)
                    spatial_gradient_scalings[i] = torch.ones(param.shape[-2:]).cuda()
                    i += 1
    else:
        spatial_gradient_scalings = None
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # spatial gradient scaling
        if spatial_gradient_scalings != None:
            if (epoch % args.sgs_epochs == 0 and epoch > args.sgs_warmup) or epoch == args.sgs_warmup:
                logger.info(" => generate_spatial_gradient_scaling_from_dependence")
                spatial_gradient_scalings = dependence.generate_spatial_gradient_scaling_from_dependence(args, model, train_loader)

            elif epoch < args.sgs_warmup:
                logger.info(" => warming up epoch")
                
            for depth, spatial_gradient_scaling in spatial_gradient_scalings.items():
                logger.info(depth)
                logger.info('\n' + str(spatial_gradient_scaling))

        # train
        metrics = train_epoch(args, epoch, model, model_ema, train_loader,
                              optimizer, loss_fn, scheduler, auxiliary_buffer,
                              dyrep, spatial_gradient_scalings)
...
def train_epoch(args,
    ...
    model.train()
    import tqdm
    for batch_idx, (input, target) in enumerate(tqdm.tqdm(loader)):
        ...
        # apply spatial gradient scalings
        if spatial_gradient_scalings != None:
            with torch.no_grad():
                i = 0
                for param in model.parameters():
                    if param.dim() == 4 and param.shape[2] > 1 and param.shape[3] > 1:
                        param.grad *= spatial_gradient_scalings[i]
                        i += 1
                assert i > 0
